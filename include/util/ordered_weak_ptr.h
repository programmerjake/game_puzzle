...
{
template <typename T>
void swap(programmerjake::game_puzzle::ordered_weak_ptr<T> &l,
          programmerjake::game_puzzle::ordered_weak_ptr<T> &r)
{
    l.swap(r);
}

template <typename T>
struct hash<programmerjake::game_puzzle::ordered_weak_ptr<T>> final
{
    size_t operator()(const programmerjake::game_puzzle::ordered_weak_ptr<T> &v) const
    {
        return v.hash();
    }
};
