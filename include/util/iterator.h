...
struct joined_iterator_value_type_helper final
{
    static_assert(
        std::is_same<
            typename std::remove_const<typename std::iterator_traits<IT1>::value_type>::type,
            typename std::remove_const<typename std::iterator_traits<IT1>::value_type>::type>::
            value,
        "can't join incompatible iterators");
    typedef typename std::
        conditional<std::is_const<typename std::iterator_traits<IT1>::value_type>::value
                        || std::is_const<typename std::iterator_traits<IT1>::value_type>::value,
                    typename std::add_const<typename std::iterator_traits<IT1>::value_type>::type,
                    typename std::iterator_traits<IT1>::value_type>::type type;
};
...
template <>
struct combined_iterator_category<std::bidirectional_iterator_tag, std::random_access_iterator_tag>
    final
{
...
template <>
struct combined_iterator_category<std::random_access_iterator_tag, std::bidirectional_iterator_tag>
    final
{
    typedef std::bidirectional_iterator_tag type;
};
...
template <typename IT1, typename IT2>
class joined_iterator final
    : public std::iterator<
          typename combined_iterator_category<std::forward_iterator_tag,
                                              typename combined_iterator_category<
                                                  typename std::iterator_traits<IT1>::
                                                      iterator_category,
                                                  typename std::iterator_traits<IT2>::
                                                      iterator_category>::type>::type,
          typename joined_iterator_value_type_helper<IT1, IT2>::type>
{
...
public:
    ...
    typename joined_iterator_value_type_helper<IT1, IT2>::type &operator*() const
    {
        static_assert(
            std::is_same<
                typename std::remove_const<typename std::iterator_traits<IT1>::value_type &>::type,
                typename std::remove_const<decltype(*std::declval<IT1>())>::type>::value,
            "unsupported return type for operator *()");
        static_assert(
            std::is_same<
                typename std::remove_const<typename std::iterator_traits<IT1>::value_type &>::type,
                typename std::remove_const<decltype(*std::declval<IT2>())>::type>::value,
            "unsupported return type for operator *()");
...
template <typename IT1, typename IT2>
joined_iterator<typename std::decay<IT1>::type, typename std::decay<IT2>::type> join_iterators(
    IT1 iterator1, IT1 iterator1end, IT2 iterator2)
{
    return joined_iterator<typename std::decay<IT1>::type, typename std::decay<IT2>::type>(
        iterator1, iterator1end, iterator2);
}
...
public:
    range(T beginV, T endV) : beginV(beginV), endV(endV)
    {
    }
...
class iterator_type_helper_has_begin<CT, typename std::enable_if<std::is_class<CT>::value>::type>
    final
{
private:
    struct yes
...
template <typename CT>
struct iterator_type<CT,
                     typename std::enable_if<iterator_type_helper_has_begin<CT>::has_begin>::type>
    final
{
...
{
    return range<typename iterator_type<CT>::type>(iterator_type<CT>::begin(container),
                                                   iterator_type<CT>::end(container));
}

template <typename CT1, typename CT2>
range<joined_iterator<typename iterator_type<CT1>::type, typename iterator_type<CT2>::type>>
    join_ranges(CT1 &&c1, CT2 &&c2)
{
    typedef joined_iterator<typename iterator_type<CT1>::type, typename iterator_type<CT2>::type>
        joined_iterator_type;
...
public:
    ...
    typedef T value_type;
    explicit unit_range_iterator(T &value) : value(std::addressof(value))
    {
    }
    unit_range_iterator() : value(nullptr)
    {
    }
