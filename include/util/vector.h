...
{
    std::int32_t x, y, z;
    constexpr VectorI(std::int32_t x, std::int32_t y, std::int32_t z) : x(x), y(y), z(z)
    {
    }

public:
    constexpr VectorI(std::int32_t v = 0) : x(v), y(v), z(v)
    {
    }
...
struct VectorF
{
    ...
    constexpr VectorF(float x, float y, float z) : x(x), y(y), z(z)
    {
    }

    constexpr VectorF(float v = 0) : x(v), y(v), z(v)
    {
    }
...
{
    return normalize(static_cast<VectorF>(v));
}

inline VectorF normalizeNoThrow(const VectorI &v)
{
    return normalizeNoThrow(static_cast<VectorF>(v));
}
...
{
template <>
struct hash<programmerjake::game_puzzle::VectorF> final
{
    hash<float> floatHasher;
    std::size_t operator()(const programmerjake::game_puzzle::VectorF &v) const
    {
...
template <>
struct hash<programmerjake::game_puzzle::VectorI> final
{
    hash<std::int32_t> int32Hasher;
    std::size_t operator()(const programmerjake::game_puzzle::VectorI &v) const
    {
